...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `${PRODUCT.userDataDirectory}${suffix}`);
...
function registerLocalScribeProtocol() {
...
const protocolRegistered = registerLocalScribeProtocol() || isOAuthSchemeRegistered();
...
function initializeDeferredManagers() {
  ...
  // Calendar OAuth and cloud-backed calendar polling are intentionally disabled.
...
async function startApp() {
  ...
  session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
    const blocked = isBlockedLocalOnlyUrl(details.url);
    if (blocked) {
      debugLogger.warn("Blocked legacy cloud request", {
        endpointHost: new URL(details.url).host,
        errorCategory: "local_only_policy",
      });
    }
...
if (gotSingleInstanceLock) {
  app.on("second-instance", async (_event, _commandLine) => {
    ...
    // LocalScribe does not consume login, billing, invitation, or upgrade deep links.
  });
